...
export const saveSettingsToFile = async (settings: GlobalSettings): Promise<void> => {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to save settings file: ${response.statusText} - ${errorText}`);
    }
    
    const responseData = await response.json();
    console.log(' [saveSettingsToFile] Settings saved to file successfully:', responseData);
    
    // Show success message
    // alert('Settings have been saved successfully to the public/settings.json file!');
  } catch (error) {
    console.error(' [saveSettingsToFile] Failed to save settings to file:', error);
    throw error;
  }
};
...
export const loadSettingsFromUrl = async (url: string): Promise<GlobalSettings> => {
  try {
    const response = await fetch(url);
    if (!response.ok) {
      throw new Error(`Failed to load settings from ${url}: ${response.statusText}`);
    }
...
export const validateSettings = (settings: any): settings is GlobalSettings => {
  ...
  const requiredProps = ['version', 'sections', 'headerSettings', 'footerSettings', 'globalStyles'];
